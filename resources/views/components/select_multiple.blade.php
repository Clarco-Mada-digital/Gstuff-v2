{{-- <select x-cloak class="hidden" id="{{$selectId}}">
  @foreach ($options as $item)
      <option value="{{$item}}">{{$item}}</option>
  @endforeach
  {{-- <template x-for="item in {{$options}}" :key="item">
    <option :value="item" x-text="item" :selected="item == '{{$selectedItem}}' "></option>
  </template>
</select> --}}

<div x-data="dropdown()" x-init="loadOptions()" class="ralative w-full flex flex-col items-center mx-auto">
