    ...
    <div class="relative">
        <div class="custom-select-wrapper">
            <select wire:model.live="selectedCanton" wire:change="{{ $chargeVille }}" id="{{ $id }}"
                class="hidden">
                <option value="" class="text-green-gs hover:bg-supaGirlRose/10 text-xs md:text-sm">{{ __('user-search.cantons') }}
                </option>
                @foreach ($cantons as $canton)
                    <option value="{{ $canton->id }}" class="text-green-gs hover:bg-supaGirlRose/10 text-xs md:text-sm">
                        {{ $canton->nom }}
                    </option>
                @endforeach
            </select>
            <div
                class="custom-select border-supaGirlRose font-roboto-slab cursor-pointer rounded-sm md:rounded-lg border border-2 bg-white px-2 py-1 md:px-4 md:py-2">
                ...
                <div class="custom-options">
                    <div class="search-container">
                        <input type="text" id="{{ $id }}-search"
                            class="border-b-1 border-supaGirlRose text-green-gs font-roboto-slab focus:ring-supaGirlRose/50 w-full rounded-sm rounded-lg bg-white px-2 py-1 md:px-4 md:py-2 text-xs md:text-sm transition-all duration-200 focus:border-transparent focus:outline-none focus:ring-2"
