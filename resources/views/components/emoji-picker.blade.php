...
<div x-data="emojiPicker()" x-init="init()" @click.away="showEmojiPicker = false" class="relative inline-block"
    data-input-selector="{{ $inputSelector }}" data-input-id="{{ $inputId }}" data-model="{{ $model }}"
    {{ $attributes->merge(['class' => 'relative']) }}>
    <!-- Emoji button -->
    <button type="button" @click="showEmojiPicker = !showEmojiPicker"
        class="text-supaGirlRose hover:text-green-gs focus:outline-none" :class="{ 'text-green-gs': showEmojiPicker }">
        ...
        <div class="h-64 overflow-y-auto p-2">
            ...
            <template x-if="!searchQuery">
                <div class="category-view">
                    @if (isset($emojiCategories) && count($emojiCategories) > 0)
                        @foreach ($emojiCategories as $category)
                            <div x-show="activeCategory === '{{ $category['slug'] }}'" class="emoji-category">
                ...
                Alpine.data('emojiPicker', () => ({
                    ...
                    insertEmoji(emoji) {
                        ...
                        if (targetInput && targetInput.type !== 'file') {
                            ...
                            const isTextInput = ['text', 'search', 'email', 'url', 'tel', 'password']
                                .includes(targetInput.type);
