...
<div class="{{ $class }} min-w-[100px] flex-1 sm:min-w-[140px] sm:flex-none">
    <input wire:model.live="{{ $model }}" class="peer hidden" type="checkbox" id="filterByDistance" name="filter_by_distance">
    <label for="filterByDistance" class="font-roboto-slab hover:bg-green-gs peer-checked:bg-green-gs border-supaGirlRose focus:ring-green-gs text-green-gs group flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border-2 bg-white sm:px-3 px-0 sm:py-2 py-1 text-center text-xs transition-all duration-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 peer-checked:text-white text-xs sm:text-xs md:text-xs lg:text-sm xl:text-sm">
