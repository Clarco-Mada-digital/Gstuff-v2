...
<div id="langue-switch-wrapper">
    ...
    @if (count($langueData) > 6)
        <div class="mt-2">
            <button type="button" id="toggle-langue" class="flex items-center gap-1 text-xs text-gray-500 hover:text-green-gs focus:outline-none">
                <svg id="toggle-icon-langue" class="w-3 h-3 text-supaGirlRose transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path id="toggle-icon-path-langue" stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
                <span id="toggle-label-langue" class="text-supaGirlRose hover:text-green-gs">{{ __('escort-search.showMore') }}</span>
            </button>
        </div>
    @endif
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    toggleBtnLangue?.addEventListener('click', () => {
        ...
        toggleLabelLangue.textContent = expandedLangue ? '{{ __('escort-search.showLess') }}' : '{{ __('escort-search.showMore') }}';
