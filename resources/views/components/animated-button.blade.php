...
@php
    ...
    $hoverBg =
        $hoverBgColor === 'complementaryColorViolet' ? 'hover:bg-complementaryColorViolet' : "hover:bg-{$hoverBgColor}";
    $border = $borderColor === 'supaGirlRose' ? 'border-supaGirlRose' : "border-{$borderColor}";
    $hoverBorder =
        $color === 'complementaryColorViolet' ? 'hover:border-complementaryColorViolet' : "hover:border-{$color}";
...
<a href="{{ $href }}" {{ $attributes->merge(['class' => $classes]) }}>
    ...
    <span
        class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 transition-opacity duration-300 hover:opacity-100"></span>
</a>
