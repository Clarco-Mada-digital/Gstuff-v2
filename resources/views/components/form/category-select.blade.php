...
<x-form.select :name="$name" :label="$label" :selected="$selected" :required="$required" :containerClass="$containerClass"
    :selectClass="$selectClass" {{ $attributes->merge(['class' => 'w-full']) }}>
    <option value="" {{ $required ? 'disabled' : '' }} {{ $selected === null ? 'selected' : '' }}>--</option>
    @foreach ($categories as $category)
        @if (is_array($category))
            <option value="{{ $category['id'] ?? $category['value'] }}"
                {{ $selected == ($category['id'] ?? $category['value']) ? 'selected' : '' }}>
