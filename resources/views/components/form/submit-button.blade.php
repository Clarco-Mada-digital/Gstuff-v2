...
<button type="submit" x-data="{ loading: false }" @submit="loading = true"
    {{ $attributes->merge([
        'class' =>
            'font-roboto-slab bg-green-gs hover:bg-green-gs/80 cursor-pointer rounded-lg p-2 text-center text-white transition-colors duration-200 flex items-center justify-center gap-2 ' .
            $class,
    ]) }}
