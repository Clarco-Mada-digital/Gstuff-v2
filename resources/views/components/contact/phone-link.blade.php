...
@if ($phone)
    <div class="group relative flex w-full items-center justify-center">
        <a @if (!$isPause) href="tel:{{ $phone }}"
            @else
                href="#"
                aria-disabled="true" @endif
            class="font-roboto-slab {{ $class }} @if ($isPause) cursor-not-allowed pointer-events-none text-gray-500 
                @else
                    hover:text-green-gs text-green-gs @endif flex items-center gap-2 font-bold transition-all duration-300 text-xs md:text-sm">
