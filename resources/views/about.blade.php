...
@section('content')
    ...
    <div class="h-30 z-0 flex w-full flex-col items-center gap-4 bg-white px-2 py-60 pt-5 text-center xl:min-h-60">
        ...
        <p class="text-sm font-roboto-slab text-textColorParagraph">{{ __('about.welcome_text') }}</p>
    </div>
    ...
    <div class="bg-supaGirlRosePastel min-h-60 w-full pb-4">
        ...
        <div
            class="mx-auto grid w-full grid-cols-1 px-10 text-center text-green-gs lg:-mt-20 lg:grid-cols-2 lg:gap-40 xl:w-[65%] font-roboto-slab">
    ...
    <div
        ...
        <x-feature-card 
            icon="images/fi_shield.png" 
            :title="__('about.erotic_entertainment')" 
            :description="__('about.erotic_entertainment_description')" 
            alt="{{ __('about.shield_icon_alt') }}"
        />
        <x-feature-card 
            icon="images/fi_zap.png" 
            :title="__('about.security_confidentiality')" 
            :description="__('about.security_confidentiality_description')" 
            alt="{{ __('about.zap_icon_alt') }}"
        />
        <x-feature-card 
            icon="images/fi_map-pin.png" 
            :title="__('about.reliable_education')" 
            :description="__('about.reliable_education_description')" 
    ...
    <div
        ...
        <div class="flex w-full flex-col gap-10">
            <h3 class="font-roboto-slab text-green-gs text-center text-5xl font-bold">{{ __('about.register_in_a_minute') }}
            </h3>
            <form action="{{ route('register') }}" method="POST" class="flex flex-col gap-4 text-sm xl:text-base">
                ...
                <x-form.input-field 
                    name="pseudo"
                    :label="__('about.username')"
                    :value="old('pseudo')"
                    autocomplete="username"
                    :error="$errors->has('pseudo')"
                    :errorMessage="$errors->first('pseudo')"
                    required
                />
                ...
                <x-form.input-field 
                    name="date_naissance"
                    type="date"
                    :label="__('about.birth_date')"
                    :value="old('date_naissance')"
                    autocomplete="bday"
                    :error="$errors->has('date_naissance')"
                    :errorMessage="$errors->first('date_naissance')"
                    required
                />
                ...
                <x-form.input-field 
                    ...
                    type="password"
                    :label="__('about.confirm_password')"
                    autocomplete="new-password"
                    required
                />

                <div class="flex flex-col gap-2 text-sm font-roboto-slab text-textColorParagraph">
