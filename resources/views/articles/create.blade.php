...
<div x-data="articleForm()" x-init="init()" class="max-w-4xl mx-auto px-4 py-8" x-cloak>
    <h1 class="text-3xl font-bold text-gray-800 mb-6">{{__('create_article.create_new_article')}}</h1>
    ...
    <form action="{{ route('articles.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        ...
        <!-- Titre -->
        <div class="mb-6">
            <label for="title" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.title')}}*</label>
            <input type="text" name="title" id="title" x-model="title" x-on:focusout="generateSlug()" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
            @error('title')
            <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{__('create_article.oops')}}!</span> {{ $message }}</p>
            @enderror
        </div>

        <!-- Slug -->
        <div class="mb-6">
            <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.slug')}}*</label>
            <input type="text" name="slug" id="slug" x-model="slug" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
        ...
        <!-- Excerpt -->
        <div class="mb-6">
            <label for="excerpt" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.excerpt')}}*</label>
        ...
        <div x-data="app()" x-init="init($refs.wysiwyg)" class="mb-6">
            <label for="content" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.content')}}*</label>
                ...
                <div class="border border-gray-200 overflow-hidden rounded-md">
                    <div class="w-full flex border-b border-gray-200 text-xl text-gray-600">
                        ...
                        <button type="button" class="outline-none focus:outline-none border-l border-r border-gray-200 w-10 h-10 hover:text-indigo-500 active:bg-gray-50" @click="format('formatBlock','P')">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mx-auto" viewBox="0 0 24 24">
                                <path fill="currentColor" d="M10 20q-.425 0-.712-.288T9 19v-5q-2.075 0-3.537-1.463T4 9t1.463-3.537T9 4h8q.425 0 .713.288T18 5t-.288.713T17 6h-1v13q0 .425-.288.713T15 20t-.712-.288T14 19V6h-3v13q0 .425-.288.713T10 20z">
                            </svg>
                        </button>
                        <button type="button" class="outline-none focus:outline-none border-r border-gray-200 w-10 h-10 hover:text-indigo-500 active:bg-gray-50" @click="format('formatBlock','H1')">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mx-auto" viewBox="-5 -7 24 24">
                        ...
                        <button type="button" class="outline-none focus:outline-none border-r border-gray-200 w-10 h-10 hover:text-indigo-500 active:bg-gray-50" @click="format('formatBlock','H2')">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mx-auto" viewBox="-4.5 -7 24 24">
                        ...
                        <button type="button" class="outline-none focus:outline-none border-r border-gray-200 w-10 h-10 mr-1 hover:text-indigo-500 active:bg-gray-50" @click="format('formatBlock','H3')">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mx-auto" viewBox="-4.5 -6.5 24 24">
        ...
        <!-- Catégorie -->
        <div class="mb-6">
            <label for="article_category_id" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.category')}}*</label>
            <select name="article_category_id" id="article_category_id" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-2">{{__('create_article.tags')}}</label>

            <!-- Champ de recherche et sélection -->
            <div class="relative mb-3">
                <input type="text" x-model="tagSearch" x-on:input.debounce.300ms="searchTags()" x-on:keydown.enter.prevent="handleTagEnter" x-ref="tagInput" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" placeholder="{{__('create_article.search_or_add_tags')}}">

                <!-- Suggestions de tags -->
                <div x-show="tagSuggestions.length > 0 && tagSearch.length > 0" x-transition class="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md border border-gray-200 max-h-60 overflow-auto">
                    <template x-for="tag in tagSuggestions" :key="tag.id">
                        <div x-on:click="addTag(tag)" class="px-4 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center">
            ...
            <div x-show="showCreateTagOption" x-transition class="mb-3">
                <div class="flex items-center justify-between bg-blue-50 p-3 rounded-md">
                    ...
                    <button type="button" x-on:click="openCreateTagModal()" class="text-sm bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
            ...
            <div class="flex flex-wrap gap-2">
                <template x-for="tag in selectedTags" :key="tag.id">
                    <div class="inline-flex items-center bg-blue-100 text-blue-800 rounded-full px-3 py-1 text-sm">
                        <span x-text="tag.name"></span>
                        <button type="button" x-on:click="removeTag(tag.id)" class="ml-2 text-blue-600 hover:text-blue-800">
                            &times;
                        </button>
        ...
        <div class="mb-6">
            <div class="flex items-center">
                <input type="checkbox" name="is_published" id="is_published" value="1" x-model="isPublished" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                <label for="is_published" class="ml-2 block text-sm text-gray-700">{{__('create_article.publish_article')}}</label>
            </div>
        </div>

        <!-- Date de publication -->
        <div x-show="isPublished" x-transition class="mb-6">
            <label for="published_at" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.publication_date')}}</label>
            <input type="datetime-local" name="published_at" id="published_at" x-model="publishedAt" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
            @error('published_at')
            <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{__('create_article.oops')}}!</span> {{ $message }}</p>
            @enderror
        </div>
    ...
    <div x-show="showTagModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div x-on:click.away="closeCreateTagModal" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h3 class="text-lg font-medium text-gray-900 mb-4">{{__('create_article.create_new_tag')}}</h3>

            <div class="mb-4">
                <label for="newTagName" class="block text-sm font-medium text-gray-700 mb-1">{{__('create_article.tag_name')}}*</label>
                <input type="text" id="newTagName" x-model="tagSearch" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
            </div>

            <div class="flex justify-end space-x-3">
                <button type="button" x-on:click="closeCreateTagModal" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
