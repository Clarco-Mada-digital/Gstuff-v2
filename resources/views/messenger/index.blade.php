...
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen" x-data="messenger()" x-init="init()">
        ...
        <div class="w-1/4 border-r bg-white flex flex-col">
            ...
            <div class="p-4 border-b">
                ...
                <div class="flex space-x-2 overflow-x-auto pb-2">
                  <template x-for="favorite in favorites" :key="favorite.id">
                    <div @click="loadChat(favorite.id)" class="flex flex-col gap-2 flex-shrink-0 cursor-pointer items-center justify-center">
                        <img 
                            :src="favorite.avatar ? `{{asset('avatars/')}}${favorite.avatar}` : '/icon-logo.png'" 
                            :alt="favorite.pseudo" 
                            class="w-12 h-12 rounded-full object-cover"
                        >
                        <span x-text="favorite.pseudo ? favorite.pseudo : favorite.prenom ? favorite.prenom : favorite.nom_salon"></span>
                    </div>
                </template>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto relative">
                <div id="search-list" class="divide-y absolute h-full w-full inset-0 bg-white z-10" x-show="searchQuery.length > 0 && !loadingContacts">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <div x-show="currentChat" class="p-4 border-b bg-white flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <img 
                    :src="currentChatUser.avatar ? `{{asset('avatars/')}}${currentChatUser.avatar}` : '/icon-logo.png'" :alt="currentChatUser.pseudo" class="w-10 h-10 rounded-full">
                    <div>
                        <h2 x-text="currentChatUser.pseudo ? currentChatUser.pseudo : currentChatUser.prenom ? currentChatUser.prenom : currentChatUser.nom_salon" class="font-semibold"></h2>
                ...
                <div class="flex space-x-2">
                    <button @click="toggleFavorite(currentChatUser.id)" class="p-2 text-gray-500 hover:text-yellow-500">
                        <i :class="{'fas fa-star text-yellow-500': isFavorite(currentChatUser.id), 'fas fa-star': !isFavorite(currentChatUser.id)}"></i>
                    </button>
                </div>
            </div>
            ...
            <div x-show='currentChat' class="border-t bg-white p-3" >
              ...
              <div x-show="preview" class="relative mb-3">
                  ...
                  <button @click="clearAttachment()" class="absolute top-2 right-2 bg-gray-800 text-white rounded-full w-6 h-6 flex items-center justify-center">
              ...
              <form @submit.prevent="sendMessage(); clearAttachment();" class="flex items-center gap-2">
                  ...
                  <div class="relative flex-1">
                      ...
                      <!-- Bouton emoji -->
                      <button type="button" @click="showEmojiPicker = !showEmojiPicker" class="absolute right-3 top-3 text-gray-400 hover:text-yellow-500">
                          <i class="far fa-smile"></i>
                      </button>
                      
                      <!-- Picker d'emoji simple -->
                      <div x-show="showEmojiPicker" @click.away="showEmojiPicker = false" class="absolute -top-5 -translate-y-full right-0 bg-white border border-gray-200 rounded-lg shadow-lg p-2 w-64 h-48 overflow-y-auto z-10">
                  ...
                  <button 
                      ...
                      :class="{'btn-gs-gradient': newMessage.trim() || fileToUpload, 'bg-gray-300 cursor-not-allowed': !newMessage.trim() && !fileToUpload}" 
            ...
            <div x-show="!currentChat" class="flex-1 flex items-center justify-center bg-gray-50">
                <div class="text-center p-6">
                    ...
                    <p class="text-gray-500">Sélectionnez une conversation ou recherchez un contact pour commencer à discuter</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
      ...
      // Écouteur pour les événements de messagerie
      // window.Echo.private(`messages.${window.Laravel.user.id}`)
      //   .listen('MessageEvent', (data) => {
      //       const messenger = Alpine.store('messenger');
      //       if (messenger.currentChat === data.message.from_id || messenger.currentChat === data.message.to_id) {
      //           const messagesList = document.getElementById('messages-list');
      //           messagesList.insertAdjacentHTML('beforeend', data.messageCard);
