...
<div class="flex group w-[60%] {{ $isSender ? 'justify-end ms-auto' : 'justify-start me-auto' }} mb-3 message-card" data-id="{{ $message->id }}">
    <div class="{{ $isSender ? 'bg-[#6366f1] text-white' : 'bg-white' }} relative rounded-lg p-3 max-w-xs lg:max-w-md shadow">
        @if($attachment)
            <img src="{{ asset(json_decode($message->attachment)) }}" class="rounded-lg mb-2 max-w-60 object-cover h-auto">
        @endif
        @if ($message->body)
            <p>{{ $message->body }}</p>
        ...
        @endif
        @if ($message->from_id === auth()->user()->id)
            <button @click="deleteMessage({{ $message->id }})"  class="hidden group-hover:block absolute text-red-500 top-[50%] -translate-y-[50%] right-[110%] cursor-pointer" data-id="{{ $message->id }}"><i class="fas fa-trash"></i></button>
        @endif
    </div>
</div>
