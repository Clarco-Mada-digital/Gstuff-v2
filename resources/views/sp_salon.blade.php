...
<div class="container flex flex-col xl:flex-row justify-center mx-auto">
  ...
  <div x-data="{}" class="min-w-1/4 flex flex-col items-center gap-3">
    ...
    <div class="w-55 h-55  -translate-y-[50%] rounded-full border-5 border-white mx-auto">
      <img
        x-on:click="$dispatch('img-modal', {  imgModalSrc:'{{ $avatar = $salon->avatar}}' ? '{{ asset('storage/avatars/'.$avatar) }}' : '{{asset('images/icon_logo.png')}}', imgModalDesc: '' })"
        class="w-full h-full rounded-full object-center object-cover" @if($avatar=$salon->avatar)
    ...
    <span class="flex items-center gap-2 font-bold font-dm-serif">({{Str::ucfirst($salon->categorie['nom'] ??
      '')}})</span>
    <a href="tel:0000000" class="flex items-center gap-2 font-bold font-dm-serif"><svg class="w-5 h-5"
  ...
  <div class="min-w-3/4 px-5 py-5">
    ...
    <div>
      ...
      <div class="min-w-3/4 px-5 py-5">
        <div class="text-right w-full text-green-gs font-dm-serif font-bold"> <a href="{{route('salons').'?selectedSalonCategories=['.$salon->categorie['id'] ?? ''.']'}}">{{Str::ucfirst($salon->categorie['nom'] ?? '')}}</a>  / <a href="{{route('salons').'?selectedSalonCanton='.$salon->canton['id'] ?? ''}}">{{Str::ucfirst($salon->canton['nom'] ?? '')}}</a> / {{Str::ucfirst($salon->profile_type ?? '')}} / {{Str::ucfirst($salon->nom_salon ?? $salon->nom_propietaire ?? '')}}</div>
