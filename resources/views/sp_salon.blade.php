...
<div class="container flex flex-col xl:flex-row justify-center mx-auto">
    ...
    <div x-data="{}" class="min-w-1/4 flex flex-col items-center gap-3">
        ...
        <div class="w-55 h-55 -translate-y-[50%] rounded-full border-5 border-white mx-auto">
            <img x-on:click="$dispatch('img-modal', {  imgModalSrc:'{{ $avatar = $salon->avatar}}' ? '{{ asset('storage/avatars/'.$avatar) }}' : '{{asset('images/icon_logo.png')}}', imgModalDesc: '' })" class="w-full h-full rounded-full object-center object-cover" @if($avatar=$salon->avatar)
        ...
        @endphp
        <a href="tel:0000000" class="flex items-center gap-2 font-bold font-dm-serif"><svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path fill="currentColor" d="M19.95 21q-3.125 0-6.187-1.35T8.2 15.8t-3.85-5.55T3 4.05V3h5.9l.925 5.025l-2.85 2.875q.55.975 1.225 1.85t1.45 1.625q.725.725 1.588 1.388T13.1 17l2.9-2.9l5 1.025V21zM16.5 11q-.425 0-.712-.288T15.5 10t.288-.712T16.5 9t.713.288t.287.712t-.288.713T16.5 11" />
            </svg>{{$salon->telephone ?? $no_phone }}</a>
        <div class="flex items-center justify-center gap-2 text-green-gs">
            <a href="{{route('escortes')}}?selectedCanton={{ $salon->canton->id }}" class="flex items-center gap-1"> <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22" fill="none">
            ...
            <button class="flex items-center gap-1"> <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-center gap-2 text-green-gs">
            <span class="flex items-center gap-1">{{__('salon_profile.recruitment')}} : {{$salon->recrutement ?? ''}}</span>
        </div>
        <hr class="w-full h-2">
        ...
        @endauth
        <button @auth x-on:click="$dispatch('loadForSender', [{{$salon->id}}])" @else data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" @endauth class="flex items-center justify-center gap-2 w-full p-2 text-green-gs text-sm rounded-lg border border-gray-400 cursor-pointer hover:bg-green-gs hover:text-white">
            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path fill="currentColor" d="M12 3c5.5 0 10 3.58 10 8s-4.5 8-10 8c-1.24 0-2.43-.18-3.53-.5C5.55 21 2 21 2 21c2.33-2.33 2.7-3.9 2.75-4.5C3.05 15.07 2 13.13 2 11c0-4.42 4.5-8 10-8m5 9v-2h-2v2zm-4 0v-2h-2v2zm-4 0v-2H7v2z" />
            </svg>
            ...
            <section>
                ...
                <div class="min-w-3/4 px-5 py-5">
                    <div class="text-right w-full text-green-gs font-dm-serif font-bold">
                        <a href="{{ route('salons') . '?selectedSalonCategories=' . ($salon->categorie['id'] ?? '') }}">
                    ...
                    <section>
                        ...
                        <div class="flex items-center justify-between gap-5 py-5">
                            ...
                            <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('salon_profile.about_me')}}</h2>
                        ...
                        <div class="flex items-center gap-10 flex-wrap">
                            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 w-full">
                                ...
                                <div class="w-full flex items-center gap-3 font-dm-serif">
                                    <img src="{{ asset('images/icons/escort_icon.svg') }}" alt="{{__('salon_profile.girls_icon')}}" />
                                    <span>{{__('salon_profile.number_of_girls')}} : {{$salon->nombre_filles ?? '-'}} {{__('salon_profile.girls')}}</span>
                                </div>
                                <div class="w-full flex items-center gap-3 font-dm-serif">
                                    <img src="{{ asset('images/icons/langue_icon.svg') }}" alt="{{__('salon_profile.language_icon')}}" />
                                    <span>
                                ...
                                <div class="w-full flex items-center gap-3 font-dm-serif">
                                    <img src="{{ asset('images/icons/tarif_icon.svg') }}" alt="{{__('salon_profile.rate_icon')}}" />
                                    @if($salon->tarif)
                                    <span>{{__('salon_profile.rates_from')}} {{$salon->tarif ?? '-'}}.-CHF </span>
                                    @else
                                    <span>{{__('salon_profile.contact_for_rates')}}</span>
                                    @endif
                                </div>
                                <div class="w-full flex items-center gap-3 font-dm-serif">
                                    <img src="{{ asset('images/icons/cart_icon.svg') }}" alt="{{__('salon_profile.payment_icon')}}" />
                                    <span>
                                ...
                                <div class="w-full flex items-center gap-3 font-dm-serif">
                                    <img src="{{ asset('images/icons/cart_icon.svg') }}" alt="{{__('salon_profile.other_contact_icon')}}" />
                                    <span>{{__('salon_profile.other_contact')}} : {{$salon->autre_contact ?? '-'}}</span>
                                </div>
                        ...
                        <div class="flex items-center justify-between gap-5 py-5">
                            ...
                            <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('salon_profile.description')}}</h2>
                            <div class="flex-1 h-0.5 bg-green-gs"></div>
                        ...
                        <div class="flex items-center justify-between gap-5 py-5">
                            ...
                            <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('salon_profile.our_professionals')}}</h2>
                            <div class="flex-1 h-0.5 bg-green-gs"></div>

                        </div>
                        <div class="w-full flex items-center gap-10 flex-wrap">
                            <span class="w-full text-center text-green-gs font-bold font-dm-serif">{{__('salon_profile.no_escort_associated')}}</span>
                        </div>
                        ...
                        <div class="w-full flex flex-col items-center justify-center font-dm-serif gap-5 text-green-gs my-3">
                            <svg class="w-25" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path fill="currentColor" d="M6 7.5a5.5 5.5 0 1 1 11 0a5.5 5.5 0 0 1-11 0M18 14c.69 0 1.25.56 1.25 1.25V16h-2.5v-.75c0-.69.56-1.25 1.25-1.25m3.25 2v-.75a3.25 3.25 0 0 0-6.5 0V16h-1.251v6.5h9V16zm-9.75 6H2v-2a6 6 0 0 1 6-6h3.5z" /></svg>
                            <p class="font-extrabold text-3xl text-center">{{__('salon_profile.login_to_see_private_content')}}</p>
                            <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" class="font-dm-serif font-bold btn-gs-gradient rounded-lg">{{__('salon_profile.login_signup')}}</button>
                        </div>
                        @endguest
                        @auth
