...
?>
<div x-data="{ 'dropdownData': '', 'currentLocale': '{{ app()->getLocale() }}' , approximite: false, getTranslatedName(nameObj) {
            if (!nameObj) return '';
    ...
    <div class="py-15 flex min-h-72 w-full flex-col items-center justify-center bg-[#E4F1F1]">
       ...
       <button data-modal-target="search-escorte-modal" data-modal-toggle="search-escorte-modal"
        ...
        <button wire:click="approximiteFunc()" 
            class="font-dm-serif hover:bg-green-gs group mt-5 flex items-center gap-2 rounded-lg border border-gray-400 bg-white p-2 text-gray-600 hover:text-white">
            {{ $approximite ? 'Filtrer par region' : 'Filtrer par distance' }}
        </button>
       </div>
    ...
    <div class="py-15 flex min-h-72 w-full flex-col items-center justify-center bg-[#E4F1F1]">
        <h1 class="font-dm-serif text-green-gs mb-5 text-center text-xl font-bold xl:text-4xl">
            {{ __('escort-search.discover_escorts') }}</h1>
            <div class="w-full max-w-2xl mx-auto mb-4 mt-1 p-4 bg-white rounded-lg shadow">
    <div class="mb-4">
...
<button 
    ...
    class="font-dm-serif hover:bg-green-gs hover:shadow-md transition-all duration-300 group mt-4 flex items-center justify-center gap-2 rounded-lg border-2 border-green-gs bg-white px-6 py-3 text-gray-700 hover:text-white hover:border-transparent w-full max-w-xs mx-auto">
