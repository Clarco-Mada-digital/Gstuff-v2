<style>
    #messagesContainer {
        ...
        max-height: 300px;
        position: relative;
        /* Assurez-vous que le conteneur est positionné relativement */
    }
    ...
    .loading {
        display: none;
        position: absolute;
        /* Position absolue pour le spinner */
        top: 50%;
        left: 50%;
    ...
    .message-received .message-attachment {
        max-width: 80%;
        /* L'image prend au maximum 80% de la largeur du conteneur */
    }
...
@if ($user)
    <div id="chatContainer" class="fixed bottom-6 right-6 z-50">
        ...
        <div id="chatWindow"
            ...
            style="display: none;">
            <!-- En-tête du chat -->
            <div class="bg-green-gs flex items-center justify-between p-4 text-white">
                <div class="flex items-center space-x-3">
                    <button id="resetSender" aria-label="Back to contacts"
                        class="hover:bg-supaGirlRose rounded-full p-1 transition-colors" style="display: none;">
                ...
                <button id="closeChat" aria-label="Close chat"
                    class="hover:bg-supaGirlRose rounded-full p-1 transition-colors">
            ...
            <div id="messagesContainerLoading"
                class="flex flex h-full w-full items-center justify-center overflow-y-auto bg-gray-50 p-4 text-center"
                style="display: none;">
            ...
            <div id="containerSpinner"
                class="flex flex-1 items-center justify-center space-y-4 overflow-y-auto bg-gray-50 p-4"
                style="display: flex;">
                <i class="fa-solid fa-spinner fa-spin text-supaGirlRose"></i>
            </div>
            ...
            <div id="messageInputContainer" class="border-t border-gray-200 bg-white p-3" style="display: none;"
                ...
                <div class="relative">
                    ...
                    <div x-show="showEmojiPicker" @click.away="showEmojiPicker = false"
                        class="border-supaGirlRose absolute bottom-full left-0 z-50 mb-2 h-64 w-64 overflow-y-auto rounded-lg border bg-white shadow-lg"
                        style="display: none;">
                        <!-- Search Bar -->
                        <div class="border-supaGirlRose border-b p-2">
                            <input type="text" x-model="searchQuery" @input="search()" placeholder="Search emojis..."
                                class="border-supaGirlRose w-full rounded border px-2 py-1 text-sm">
                        </div>
                        ...
                        <div class="max-h-40 overflow-y-auto p-2">
                            ...
                            <template x-if="!searchQuery || searchResults.length > 0">
                                <div class="grid grid-cols-8 gap-1">
                                    <template x-for="emoji in (searchQuery ? searchResults : filteredEmojis)"
                                        :key="emoji.char">
                                        <button @click="insertEmoji(emoji.char)" :title="emoji.name"
                                            class="flex items-center justify-center rounded p-1 text-xl hover:bg-gray-100"
                    ...
                    <div class="flex">
                        ...
                        <input type="text" id="messageInput" placeholder="{{ __('chat.type_message') }}"
                            class="border-green-gs focus:ring-green-gs flex-1 rounded-full border px-4 py-2 text-sm focus:border-transparent focus:outline-none focus:ring-2">



                        <button @click="showEmojiPicker = !showEmojiPicker" type="button"
                            class="hover:text-green-gs rounded-full p-2 text-gray-500 focus:outline-none">
                            <i class="far fa-smile text-green-gs text-lg"></i>
                        </button>
                        <button id="sendMessage"
                            class="bg-green-gs hover:bg-green-gs/80 rounded-full px-3 py-2 text-white transition-colors focus:outline-none">
                            <i class="far fa-paper-plane h-4 w-4 text-lg"></i>
                        </button>
        ...
        <div id="imageModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/50">
            <span
                class="bg-supaGirlRose hover:bg-green-gs/80 absolute right-4 top-4 cursor-pointer rounded-full p-1 px-4 py-2 text-white transition-colors hover:text-gray-400">
                <i class="fa-solid fa-xmark text-sm text-white"></i>
            </span>
            <div class="flex max-h-[80vh] max-w-[80vw] items-center justify-center">
            ...
            contactElements.forEach(contactElement => {
                ...
                if (unreadCount > 0) {
                    if (!badge) {
                        ...
                        badge.className =
                            'absolute -right-1 -top-1 inline-flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-red-500 text-xs font-bold text-white';
            ...
            fetch('/api/fetch-contacts')
                ...
                .then(data => {
                    ...
                    if (data.contacts === 'No contacts found') {
                        contactsContainer.innerHTML =
                            '<p class="text-center">{{ __('chat.no_contacts_found') }}</p>';
                    } else {
                ...
                .then(data => {
                    ...
                    if (data.messages && data.messages.data && Array.isArray(data.messages.data)) {
                        ...
                        messages.forEach(message => {
                            ...
                            messageElement.className = message.from_id === currentUserId ?
                            ...
                            if (message.attachment) {
                                ...
                                let cleanUrl = message.attachment.replace(/^"|"$/g, '').replace(
                                    /[\/\\]+/g, '/').replace(/^\/+/, '');
                        ...
                        messagesContainer.innerHTML =
                            '<p class="text-center">{{ __('chat.no_messages_yet') }}</p>';
                    }
