...
<div x-data="{ open: @entangle('open') }" class="fixed bottom-10 right-10" x-cloak>
    ...
    <div x-show="!open">
        <button wire:click="toggleChat" class="bg-blue-500 text-white p-3 rounded-full shadow-lg cursor-pointer">
            <span class="text-xl"><svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7" viewBox="0 0 24 24"><!-- Icon from All by undefined - undefined --><path fill="currentColor" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12c0 1.6.376 3.112 1.043 4.453c.178.356.237.763.134 1.148l-.595 2.226a1.3 1.3 0 0 0 1.591 1.592l2.226-.596a1.63 1.63 0 0 1 1.149.133A9.96 9.96 0 0 0 12 22"/></svg></span>
        </button>
    </div>
    ...
    <div x-show="open" @keydown.window.escape="open = false" class="flex flex-col items-center justify-between w-96 h-100 bg-white shadow-lg rounded-lg mb-5">
        <div class="flex justify-between items-center w-full p-4 bg-blue-500 text-white">
            <div class="flex items-center gap-2">
                @if ($userReceved)
                <img src="@if($userReceved->avatar)
                    {{ asset('storage/avatars/'.$userReceved->avatar) }}
                @else
        ...
        <div  x-data="{scrollToBottom() {setTimeout(() => {this.$el.scrollTop = this.$el.scrollHeight;}, 100);}}"
            ...
            <div class="wsus__single_chat_area message-card" data-id="{{ $message->id }}">
                <div class="wsus__single_chat {{ $message->from_id === auth()->user()->id ? 'chat_right' : '' }}">
                    <a class="venobox" x-on:click.stop="$dispatch('img-modal', {  imgModalSrc: '{{asset($imagePath)}}', imgModalDesc: '' })">
            ...
            <div class="wsus__single_chat_area message-card" data-id="{{ $message->id }}">
                <div class="wsus__single_chat {{ $message->from_id === auth()->user()->id ? 'chat_right' : '' }}">
                    ...
                    <span class="time"> {{ timeAgo($message->created_at) }}</span>
                    @if ($message->from_id === auth()->user()->id)
                    <a wire:click='deleteMessage({{$message->id}})' class="action dlt-message" data-id="{{ $message->id }}" href="#"><i class="fas fa-trash"></i></a>
                    @endif
                </div>
            </div>
            @endif
