<div class="w-full flex flex-col items-center justify-center px-5 py-5 gap-2">
  ...
  <form wire:submit.prevent="search" class="w-full flex flex-col gap-5">
      ...
      <div class="w-full flex flex-col md:flex-row items-center justify-center text-sm xl:text-base gap-2 mb-3">
          <select wire:model.live.debounce.500ms="selectedCanton" id="small" class="block w-1/3 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-amber-500 focus:border-amber-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500">
          ...
          <select wire:model.live.debounce.500ms="selectedVille" id="small" class="block w-1/3 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-amber-500 focus:border-amber-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500" {{ $villes->isEmpty() ? 'disabled' : '' }}>
          ...
          <!-- Sélection du genre -->
          <select wire:model.live.debounce.500ms="selectedGenre" id="small" class="block w-1/3 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-amber-500 focus:border-amber-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500">
      ...
      <div class="flex flex-wrap items-center justify-center gap-2 mb-3 font-bold text-sm xl:text-base">
          @foreach ($salonCategories as $categorie)
              <div wire:key="salon-{{ $categorie->id }}">
                  <input wire:model.live.debounce.500ms="selectedCategories" type="checkbox" name="categories[]" id="categorie{{ $categorie->id }}" value="{{ $categorie->id }}" class="hidden peer">
