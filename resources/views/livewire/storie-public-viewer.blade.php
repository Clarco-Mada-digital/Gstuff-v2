<div>
    @if (count($storiesData) > 0)
            ...
            @if (count($storiesData) > 0)
                @foreach ($storiesData as $index => $story)
                    <div class="relative h-24 w-24">
                        <div class="border-green-gs relative h-24 w-24 cursor-pointer overflow-hidden rounded-full border-2"
                            onclick="openStory({{ $index }})">
                            @if ($story['media_type'] === 'image')
                                <img src="{{ $story['media_url'] }}" alt="Story" class="h-full w-full object-cover"
                                    loading="lazy">
                            @else
                                <video class="h-full w-full object-cover" muted loop playsinline>
        ...
        <div id="storyModal"
            class="fixed inset-0 z-50 hidden flex-col items-center justify-center bg-black bg-opacity-90 p-4">
            <!-- Barre de progression -->
            <div class="mb-4 w-full max-w-2xl">
                <div class="flex gap-1">
                    @foreach ($storiesData as $index => $story)
                        <div class="h-1 flex-1 overflow-hidden rounded-full bg-gray-600">
                            <div id="progress-{{ $index }}" class="bg-green-gs h-full"
                                style="width: 0%; transition: width 15s linear;"></div>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Contenu de la story -->
            <div class="relative flex h-[80vh] w-full max-w-2xl items-center justify-center">
                <img id="storyImage" src="" alt="Story"
                    class="hidden max-h-full max-w-full cursor-pointer object-contain" onclick="togglePause()">
                <video id="storyVideo" src="" class="hidden max-h-full max-w-full cursor-pointer object-contain"
                    autoplay playsinline onended="videoEnded()" onclick="togglePause()"></video>
            </div>
