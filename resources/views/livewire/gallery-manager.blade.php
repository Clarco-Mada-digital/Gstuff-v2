<div class="w-full" x-data="gallery()" x-init="initGallery()" x-cloak @keydown.left="if(fullscreen) navigateMedia(-1)" @keydown.right="if(fullscreen) navigateMedia(1)">
    ...
    <div class="flex justify-between items-center gap-3 mb-6 text-green-gs">
        <h2 class="font-dm-serif font-bold text-2xl">Galerie @if ($isPublic == false) privée @endif</h2>
    ...
    <div x-show="showDeleteModal" 
        ...
        <div class="w-full max-w-md bg-white rounded-lg shadow-xl overflow-hidden">
            <div class="p-6">
                ...
                <p class="text-gray-600 mb-6">Êtes-vous sûr de vouloir supprimer ce média ? Cette action est irréversible.</p>
    ...
    <template x-if="viewMode === 'grid'">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            <div class="relative group rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 aspect-square">
                ...
                @else
                <div class="relative w-full h-full bg-gray-800 flex items-center justify-center cursor-pointer"
                     @click="currentMedia = { 
                        type: 'video', 
                        url: '{{ $media->url }}', 
                        title: '{{ $media->title }}',
                    ...
                    <img 
                        src="{{ $media->thumbnail_url ?? asset('images/icon_logo.png') }}" 
                        alt="{{ $media->title }}"
                ...
                <div class="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <button @click.stop="mediaToDelete = {{ $media->id }}; deleteModalTitle = 'Supprimer {{ addslashes($media->title) }}'; showDeleteModal = true" 
            ...
            <div class="col-span-full text-center py-12 text-gray-500">
                ...
                <button @click="$wire.openModal()" class="btn-primary mt-4">
                    <i class="fas fa-plus mr-2"></i> Ajouter votre premier média
                </button>
                @endif
            </div>
            @endif
    ...
    <template x-if="viewMode === 'list'">
        <div class="space-y-3">
            ...
            <div class="flex items-center p-3 bg-white rounded-lg shadow hover:shadow-md transition-shadow duration-300 group">
                <div class="flex-shrink-0 w-16 h-16 rounded-md overflow-hidden cursor-pointer"
                     @click="currentMedia = { 
                        type: '{{ $media->type }}', 
                        url: '{{ $media->url }}', 
                        title: '{{ $media->title }}',
                ...
                <div class="ml-4 flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 truncate">{{ $media->description ?: 'Aucune description' }}</p>
    ...
    <x-modal wire:model="showModal" maxWidth="2xl">
        <div class="p-6">
            <h3 class="text-xl font-semibold mb-4">
                {{ $selectedMedia ? 'Modifier le média' : 'Ajouter un nouveau média' }}
            </h3>
            ...
            <form wire:submit.prevent="saveMedia">
                <div class="space-y-4">
                    ...
                    @if(!$selectedMedia)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Média *</label>
                        <div x-data="{ isUploading: false, progress: 0, isDragging: false }" 
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center" :class="{ 'border-blue-500 bg-blue-50': isDragging, 'border-gray-300': !isDragging }">
                                ...
                                <label for="galleryUpload" class="cursor-pointer">
                                    ...
                                    <p class="text-sm text-gray-600">Glissez-déposez vos fichiers ou cliquez pour sélectionner</p>
                                    <p class="text-xs text-gray-500 mt-1">Formats supportés: JPG, PNG, WEBP, MP4 (max 100MB)</p>
                                </label>
                            </div>
                            ...
                            <template x-if="isUploading">
                                <div class="mt-2">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1" x-text="`Upload en cours... ${progress}%`"></p>
                                </div>
                            </template>
                    ...
                    <div class="flex items-center">
                        ...
                        <label for="isPublic" class="ml-2 text-sm text-gray-700">Rendre ce média public</label>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    ...
                    <button type="submit" class="btn-gs-gradient rounded">
                        {{ $selectedMedia ? 'Mettre à jour' : 'Enregistrer' }}
                    </button>
                </div>
            </form>
        </div>
        ...
        <template x-if="currentMedia">
            <div class="max-w-6xl w-full relative">
                ...
                <button @click.stop="navigateMedia(-1)"
                        class="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-3 text-white hover:text-blue-300 transition-colors"
                ...
                <!-- Bouton suivant -->
                <button @click.stop="navigateMedia(1)"
                        class="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-3 text-white hover:text-blue-300 transition-colors"
