<div x-data="{
    ...
    <div
        class="flex flex-col space-y-4 py-5 sm:flex-row sm:items-center sm:justify-between md:flex-row md:items-center md:justify-between md:space-y-0">
        ...
        @if ($geo)
            <div class="relative">
                ...
                <div x-show="isActive" x-transition:enter="transition-opacity ease-out duration-300"
                    x-transition:leave="transition-opacity ease-in duration-200" x-data="{ showPing: true }"
                    x-init="setTimeout(() => showPing = false, 3000)" x-show="showPing"
                    class="bg-green-gs absolute inset-0 rounded-full opacity-75"
                    :class="{ 'animate-ping-once': showPing }" style="width: 3rem; height: 3rem; --tw-bg-opacity: 0.7;">
                </div>

                <button @click="toggleGeolocation"
                    class="focus:ring-green-gs relative flex h-12 w-12 transform items-center justify-center rounded-full shadow-lg transition-all duration-300 ease-in-out hover:scale-110 focus:outline-none focus:ring-4 focus:ring-opacity-50"
                    :class="isActive ?
                        'bg-gradient-to-br from-supraGirlRosePastel/50 to-supraGirlRosePastel/100 hover:from-supraGirlRosePastel/150 hover:to-supraGirlRosePastel/200' :
                        'bg-gradient-to-br from-green-gs-300 to-green-gs-400 hover:from-green-gs-400 hover:to-green-gs-500'"
                    ...
                    <svg x-show="isActive" class="h-6 w-6 transform text-white transition-all duration-300"
                    ...
                    <svg x-show="!isActive" class="h-6 w-6 transform text-gray-700 transition-all duration-300"
                        fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 16.55c-2.0-2.69-5-7.05-5-9.55 0-2.76 2.24-5 5-5s5 2.24 5 5c0 2.49-3 6.86-5 9.55z M12 6c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z" />
                    </svg>

                    <!-- Animated checkmark when active -->
                    <div x-show="isActive" class="absolute inset-0 flex items-center justify-center">
                        <div class="bg-green-gs h-3 w-3 animate-ping rounded-full"></div>
                    </div>
                </button>

                <!-- Tooltip text for better UX -->
                <div x-show="!isActive"
                    class="bg-supraGirlRose pointer-events-none absolute left-1/2 top-full mt-2 -translate-x-1/2 transform whitespace-nowrap rounded px-2 py-1 text-xs text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
    ...
    @if ($maxAvailableDistance > 0)
        <div class="border-green-gs mb-6 h-[10vh] rounded-lg border bg-white px-4 py-3 shadow-sm" x-show="isActive">
            <div class="mb-2 flex items-center justify-between">
                <label for="distance"
                    class="text-textColorParagraph font-roboto-slab block text-sm font-medium">{{ __('proximity.max_distance') }}</label>
                <div class="flex items-center">
                    <span
                        class="bg-green-gs font-roboto-slab mr-2 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium text-white">
                        <span x-text="$wire.selectedDistance.toFixed(1)"></span> km
                    </span>
                    <div wire:loading wire:target="selectedDistance" class="flex items-center">
                        <svg class="text-green-gs h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg"
    ...
    <div id="escortsContainer">
                ...
                @foreach ($escorts as $escort)
                    <div class="escort-card" data-distance="{{ $escort['distance'] ?? '' }}">
                        <livewire:escort-card wire:key="escort-{{ $escort['escort']['id'] }}"
                            name="{{ $escort['escort']['prenom'] ?? '' }}" canton="{{ $escort['canton']->nom ?? '' }}"
                            ville="{{ $escort['ville']->nom ?? '' }}" avatar="{{ $escort['escort']['avatar'] ?? '' }}"
                            escortId="{{ $escort['escort']['id'] ?? '' }}" distance="{{ $escort['distance'] ?? '' }}"
        ...
        @else
            <div class="flex items-center justify-center py-10">
                <p class="text-textColorParagraph font-roboto-slab text-lg">{{ __('proximity.no_results_found') }}</p>
            </div>
        @endif
    </div>
