<div x-data="{ storyForm: false }">
    ...
    <div class="font-roboto-slab flex items-center justify-between gap-5 py-5">
        ...
        <button x-on:click="storyForm = true"
            class="text-green-gs hover:text-green-gs hover:bg-fieldBg bg-supaGirlRose flex cursor-pointer items-center gap-2 rounded-md px-2 py-1 sm:px-5 sm:py-2 text-xs sm:text-sm ">
            {{ __('profile.add') }}
    ...
    <div class="container mx-auto p-4">
        <div class="flex items-center gap-10">
            ...
            <div id="storiesContainer" class="flex flex-wrap space-x-4 space-y-4 px-2 py-4">
                <!-- Les stories seront ajoutées ici dynamiquement -->
            </div>
        </div>

        <!-- Message de confirmation -->
        <div id="confirmationMessage" class="mt-4 hidden rounded-lg bg-green-100 p-4 text-green-700">
            <!-- Le message de confirmation sera affiché ici -->
        </div>

        <!-- Modal de visualisation -->
        <div id="storyModal"
            class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black bg-opacity-90 p-4">
            <div class="relative flex h-full w-full max-w-2xl items-center justify-center">
                <img id="storyImage" src="" alt="Story" class="hidden max-h-full max-w-full object-contain">
                <video id="storyVideo" src="" class="hidden max-h-full max-w-full object-contain" controls
                    autoplay></video>
            </div>
            ...
            <!-- Bouton précédent -->
            <button id="previousButton" onclick="previousStory()"
            ...
            <!-- Bouton suivant -->
            <button id="nextButton" onclick="nextStory()"
        ...
        <div x-cloak x-show="storyForm" x-transition.opacity.duration.300ms x-trap.inert.noscroll="storyForm"
            x-on:keydown.esc.window="storyForm = false" x-on:click.self="storyForm = false"
            ...
            <!-- Modal Dialog -->
            <div x-show="storyForm" x-data="mediaViewer('')" x-transition:enter="transition ease-out duration-200"
                ...
                <!-- Modal Content -->
                <div class="p-6">
                    <h3 id="story-modal-title"
                        class="text-green-gs font-roboto-slab mb-6 text-center text-xl font-bold">
                    ...
                    <form action="{{ route('stories.store') }}" method="post" enctype="multipart/form-data"
                        ...
                        <div class="flex justify-center">
                            <div
                                class="relative h-64 w-full overflow-hidden rounded-lg border-2 border-dashed border-gray-300 bg-gray-50">
                                <template x-if="!mediaUrl">
                                    <div class="flex h-full flex-col items-center justify-center p-4 text-center">
                                        ...
                                        <span
                                            class="mt-2 block text-sm text-gray-600">{{ __('storie_media_viewer.media_preview') }}</span>
                                    </div>
                                </template>
                                <template x-if="mediaUrl">
                                ...
                                <template x-if="mediaUrl && !isImage">
                                    <div class="relative h-full w-full">
                                        <video :src="mediaUrl" class="h-full w-full object-contain" controls
                                        ...
                                        <div
                                            ...
                                            <button type="button"
                                                class="rounded-full bg-white bg-opacity-80 p-3 shadow-lg transition-all hover:bg-opacity-100"
                                                @click="const video = $event.target.closest('div').querySelector('video'); video.paused ? video.play() : video.pause();">
                                                <svg x-show="!$el.closest('div').querySelector('video').paused"
                                                    class="h-8 w-8 text-gray-700" fill="currentColor"
                                                    viewBox="0 0 20 20">
                        ...
                        <div class="mt-4">
                            <label
                                class="hover:border-green-gs hover:bg-green-gs/50 flex cursor-pointer items-center justify-between rounded-lg border-2 border-dashed border-gray-300 bg-white p-4 transition-colors">
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-2 text-sm text-gray-700">
                                        <span
                                            x-text="mediaUrl ? `{{ __('storie_media_viewer.change_file') }}` : `{{ __('storie_media_viewer.select_file_Title') }}`"></span>
                                        <span
                                            class="block text-xs text-gray-500">{{ __('storie_media_viewer.select_file') }}</span>
                                    </span>
                                </div>
                        ...
                        <div class="mt-6 flex justify-end space-x-3">
                            ...
                            <button type="submit"
                                class="bg-green-gs hover:bg-green-gs/90 focus:ring-green-gs/50 inline-flex items-center rounded-lg px-4 py-2 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2">
                                <svg x-show="!mediaUrl" class="-ml-0.5 mr-2 h-4 w-4" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<script>
    ...
    function displayStories() {
        ...
        if (stories.length === 0) {
            container.innerHTML =
                '<div class="w-full py-8 text-center text-gray-500">{{ __('storie_media_viewer.no_stories') }}</div>';
            return;
        }
