...
<div class="relative flex items-center justify-center flex-col gap-8 w-full px-3 py-20 lg:h-[418px] bg-no-repeat" style="background: url('images/Hero image.jpeg') center center /cover;">
    ...
    <div class="flex items-center justify-center flex-col z-10">
        <h2 class="[text-shadow:_2px_6px_9px_rgb(0_0_0_/_0.8)] lg:text-6xl md:text-5xl text-4xl text-center font-semibold text-white font-cormorant">{{ __('home.meetings')}} <span class="text-amber-400">{{ __('home.elegant_discreet')}}</span> {{ __('home.in_switzerland')}}</h2>
    </div>
    ...
    <div class="z-10">
        <a href="{{ route('escortes') }}" type="button" class="flex items-center justify-center gap-2 btn-gs-gradient text-black font-bold focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-sm px-4 py-2 text-center dark:focus:ring-blue-800">{{ __('home.see_all')}} <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
<div class="mt-10 container m-auto px-5 overflow-hidden">
    ...
    <div x-data="{ viewEscorte: true }" x-cloak>
        ...
        <div x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-show="!viewEscorte" class="relative w-full mx-auto flex flex-col items-center justify-center mt-4">
            <h3 class="font-dm-serif text-green-gs font-bold text-4xl text-center">{{ __('home.our_salons')}}</h3>
            <div id="OurSalonContainer" class="w-full min-h-30 flex items-center justify-start overflow-x-auto flex-nowrap mt-5 mb-4 px-10 gap-4" style="scroll-snap-type: x proximity; scrollbar-size: none; scrollbar-color: transparent transparent">
                ...
                @endforeach
                @if($salons == '[]')
                <h3 class="w-full font-dm-serif text-3xl text-center text-green-gs">{{ __('home.no_salon_yet')}}</h3>
                @endif
            </div>
    ...
    <div class="relative w-full mx-auto flex flex-col items-center justify-center mt-4">
        <h3 class="font-dm-serif text-green-gs font-bold text-3xl lg:text-4xl text-center">{{__('home.looking_for_fun')}}</h3>
        ...
        <div class="z-10 mb-6">
            <a href="{{ route('escortes') }}" type="button" class="flex items-center justify-center gap-2 btn-gs-gradient text-black font-bold focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg 
            text-sm px-4 py-2 text-center dark:focus:ring-blue-800">{{__('home.see_all')}} <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path fill="currentColor" d="m8.006 21.308l-1.064-1.064L15.187 12L6.942 3.756l1.064-1.064L17.314 12z" /></svg>
            </a>
        </div>
    </div>
...
<div class="relative flex flex-col items-center justify-center py-10 w-full" style="background: url('images/girl_deco_image.jpg') center center /cover">
    ...
    <h3 class="font-dm-serif text-green-gs text-2xl md:text-3xl lg:text-4xl xl:text-5xl my-4 mx-2 text-center font-bold z-10">{{__('home.find_escorts')}}</h3>
    <div class="flex flex-col w-full px-4 md:flex-row items-center justify-center gap-2 py-6 z-10">
        ...
        <div class="w-full lg:w-[367px] lg:h-[263px] bg-[#618E8D] p-3 flex flex-col text-white text-2xl lg:text-4xl font-bold items-center justify-center gap-3">
            <span class="text-center font-dm-serif w-[70%]">{{__('home.partners_count')}}</span>
            <span class="text-center text-sm lg:text-base font-normal w-[75%] mx-auto">{{__('home.verified_profiles')}}</span>
        </div>
        <div class="w-full lg:w-[367px] lg:h-[263px] bg-[#618E8D] p-3 flex flex-col text-white text-2xl lg:text-4xl font-bold items-center justify-center gap-3">
            <span class="text-center font-dm-serif w-[70%]">{{ __('home.amateurs_count')}}</span>
            <span class="text-center text-sm lg:text-base font-normal w-[75%] mx-auto">{{ __('home.amateur_experiences')}}</span>
        </div>
        <div class="w-full lg:w-[367px] lg:h-[263px] bg-[#618E8D] p-3 flex flex-col text-white text-2xl lg:text-4xl font-bold items-center justify-center gap-3">
            <span class="text-center font-dm-serif w-[70%]">{{ __('home.professional_salons_count')}}</span>
            <span class="text-center text-sm lg:text-base font-normal w-[75%] mx-auto">{{__('home.professional_salons_offer')}}</span>
        </div>
...
<div class="bg-white py-10 w-full overflow-x-hidden flex items-center justify-center flex-col gap-10">
    <h3 class="text-xl md:text-4xl lg:text-5xl font-dm-serif text-green-gs font-bold">{{ __('home.become_escort_title')}}</h3>
    ...
    <div class="relative grid grid-cols-3 gap-5 text-green-gs  text-xs lg:text-lg text-wrap italic font-normal mt-20 mx-0 px-2">
        ...
        @endforeach
        <div class="lg:w-52 w-30 text-wrap text-center">{{__('home.send_selfies')}} <a href="http://escort-gstuff@gstuff.ch" class="text-amber-500">escort-gstuff@gstuff.ch</a></div>
        <div class="lg:w-52 w-30 text-wrap text-center"> {{__('home.photo_shoot_appointment')}}</div>
        <div class="lg:w-52 w-30 text-wrap text-center">{{__('home.publish_profile')}}</div>
    </div>
</div>
...
<div class="container mx-auto flex flex-col items-center justify-center gap-10">
    <h3 id="FAQ" class="text-3xl lg:text-5xl font-dm-serif text-green-gs">{{__('home.frequent_questions')}}</h3>
...
<div class="lg:container mx-auto my-10">
    <div class="flex items-center justify-between my-10 px-5 lg:px-20">
        <h3 class="font-dm-serif text-2xl lg:text-4xl text-green-800 font-bold">{{__('home.glossary_articles')}}</h3>
        <div class="z-10 w-auto">
            <a href="#" type="button" class="flex items-center justify-center gap-2 btn-gs-gradient text-black font-bold focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg px-4 py-2 text-center text-sm lg:text-base dark:focus:ring-blue-800">{{__('home.see_more_articles')}} <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path fill="currentColor" d="m8.006 21.308l-1.064-1.064L15.187 12L6.942 3.756l1.064-1.064L17.314 12z" /></svg>
            </a>
        </div>
    </div>
