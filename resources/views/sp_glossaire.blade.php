...
@section('content')
    ...
    <div
        ...
        <div class="w-full text-justify lg:w-2/3 font-roboto-slab">
            {!! $glossaire->content !!}
        </div>
        <div class="w-full lg:w-1/3">
            <div id="accordion-collapse" data-accordion="collapse">
                <h2 id="accordion-collapse-heading-1">
                    <button type="button"
                        class="font-roboto-slab border-b-1 flex w-full items-center justify-between gap-3 rounded-t-xl border border-supaGirlRosePastel p-4 text-xl 
                        font-bold text-green-gs hover:bg-supaGirlRose/10 hover:text-green-gs sm:text-2xl md:text-3xl bg-supaGirlRose"
                ...
                <div id="accordion-collapse-body-1" class="hidden" aria-labelledby="accordion-collapse-heading-1">
                    <div class="border border-supaGirlRosePastel bg-fieldBg p-4 text-green-gs">
                        <ul class="mb-2 flex flex-col gap-3 text-green-gs">
                            @foreach ($glossaires as $glossaire)
                                <li class="hover:bg-supaGirlRose/10 p-2"><a href="{{ route('glossaires.show', $glossaire->slug) }}"
                                        class="text-sm font-bold font-roboto-slab  hover:text-green-gs">{{ $glossaire->title }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
    ...
    <div class="container mx-auto my-8 flex flex-col gap-6 sm:my-10">
        <div class="flex flex-col items-center justify-between px-4 sm:flex-row sm:px-6 md:px-16 lg:px-60">
            <h3 class="font-roboto-slab text-green-gs mb-4 text-3xl font-bold sm:mb-0 sm:text-4xl md:text-5xl">{{ __('glossary.glossary') }}</h3>
            <div class="z-10">
