...
@section('content')
    ...
    <div
        ...
        <div class="font-roboto-slab w-full text-justify lg:w-2/3 text-xs md:text-sm">
            {!! $glossaire->content !!}
        </div>
            ...
            <div id="accordion-collapse" data-accordion="collapse">
                <h2 id="accordion-collapse-heading-1">
                    <button type="button"
                        ...
                        <span class="text-sm md:text-xl">{{ __('glossary.glossary') }}</span>
                ...
                <div id="accordion-collapse-body-1" class="hidden" aria-labelledby="accordion-collapse-heading-1">
                    <div class="border-supaGirlRosePastel bg-fieldBg text-green-gs border p-4">
                        <ul class="text-green-gs mb-2 flex flex-col gap-1 md:gap-3">
                            @foreach ($glossaires as $glossaire)
                                <li class="hover:bg-supaGirlRose/10 "><a
                                        href="{{ route('glossaires.show', $glossaire->slug) }}"
                                        class="font-roboto-slab hover:text-green-gs text-xs md:text-sm font-bold">{{ $glossaire->title }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between px-4 sm:px-6 md:px-16 lg:px-60 gap-2">
    <h3 class="font-roboto-slab text-green-gs text-xl md:text-2xl font-bold">
