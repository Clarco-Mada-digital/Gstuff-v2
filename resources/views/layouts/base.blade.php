...
<head>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/alpinejs-focus/3.14.9/cdn.min.js"
        integrity="sha512-cbnb6RLeH6MQInYpFTaBZhIvqoV1SQMCT7gS/lcz5U5EbifRo1yHOlDHZOP0i/2oo7G9rSNpSGuqHdK3jByq2Q==" crossorigin="anonymous" referrerpolicy="no-referrer" defer></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/alpinejs-intersect/3.14.9/cdn.min.js"
        integrity="sha512-zUi+vHO/A+Mh3PQDYpAhzo3GGnrSTdOdyUkFby6I2p+k5kOhVNDMJMnhaJgZtecorfhS/+Y5PbkDu7iWsnk8+A==" crossorigin="anonymous" referrerpolicy="no-referrer" defer></script>
    <!-- Alpine Core -->
...
<body
    class="w-full min-h-[100vh] overflow-x-hidden relative antialiased font-dm text-sm md:text-base font-normal transition-all">
    @if (session('success'))
        <div id="sessionAlert"
            class="absolute top-3 ring-2 flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
            role="alert">
    ...
    <div id="loader" class="absolute top-0 left-0 w-full h-full bg-white z-50">
        <div
            class="fixed top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] w-full h-screen flex items-center gap-4 justify-center text-5xl xl:text-6xl">
            ...
            <div x-on:click.stop="imgModal = false" x-transition:enter="transition ease-out duration-300"
                ...
                <div @click.stop class="flex flex-col w-full max-w-[90vw] max-h-[90vh] overflow-auto">
                    ...
                    <div class="w-full flex items-center justify-center p-2">
                        <img class="object-contain object-center w-full max-w-[75vw] max-h-[80vh]"
                            :src="imgModalSrc">
        ...
        <div class="relative p-4 w-[95%] lg:w-[60%]  max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div
                    ...
                    <button type="button"
                        class="end-2.5 text-green-gs bg-transparent hover:bg-gray-200 hover:text-amber-400 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                ...
                @guest
                    <div x-data="loginForm" class="relative p-4 md:p-5">
                        ...
                        <div x-show="message"
                            class="flex items-center p-4 mb-4 text-sm  border rounded-lg dark:bg-gray-800"
                            :class="status ?
...
<form x-show="showloginForm" x-on:submit.prevent="submitForm()" id="loginForm" class="space-y-4" action="{{ route('login') }}" method="POST">
    @csrf
    <div>
        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('login_form.email') }} *</label>
    ...
    <div class="relative" x-data="{ 'pwdShow': true }">
        <label for="conex_pass" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('login_form.password') }} *</label>
        ...
        <div class="absolute bottom-3 right-0 pr-3 flex items-center text-sm leading-5">
            <svg class="h-4 text-gray-700" fill="none" @click="pwdShow = !pwdShow" :class="{ 'hidden': !pwdShow, 'block': pwdShow }" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 576 512">
                <path fill="currentColor" d="M572.52 241.4C518.29 135.59 410.93 64 288 64S57.68 135.64 3.48 241.41a32.35 32.35 0 0 0 0 29.19C57.71 376.41 165.07 448 288 448s230.32-71.64 284.52-177.41a32.35 32.35 0 0 0 0-29.19zM288 400a144 144 0 1 1 144-144 143.93 143.93 0 0 1-144 144zm0-240a95.31 95.31 0 0 0-25.31 3.79 47.85 47.85 0 0 1-66.9 66.9A95.78 95.78 0 1 0 288 160z">
                </path>
            </svg>
            <svg class="h-4 text-gray-700" fill="none" @click="pwdShow = !pwdShow" :class="{ 'block': !pwdShow, 'hidden': pwdShow }" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 640 512">
    ...
    <div class="flex justify-between">
        <div class="flex items-start">
            ...
            <label for="remember" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('login_form.remember_me') }}</label>
        </div>
        <a href="#" x-on:click="showloginForm = false" class="text-sm text-green-gs hover:underline hover:text-amber-300 dark:text-green-gs">{{ __('login_form.forgot_password') }}</a>
    </div>
    ...
    <div class="flex flex-col gap-2 py-5 w-full text-sm lg:text-base items-center justify-center transition-all">
        <a href="{{ route('registerForm') }}" class="w-full p-3 text-center border border-amber-300 hover:bg-amber-300 hover:text-green-gs">{{ __('login_form.register_free') }}</a>
        <a href="{{ route('escort_register') }}" class="w-full p-3 text-center border border-amber-300 hover:bg-amber-300 hover:text-green-gs">{{ __('login_form.register_escort') }}</a>
        <a href="{{ route('salon_register') }}" class="w-full p-3 text-center border border-amber-300 hover:bg-amber-300 hover:text-green-gs">{{ __('login_form.register_professional') }}</a>
    </div>
</form>
...
<form x-show="!showloginForm" x-on:submit.prevent="submitForm(true)" id="resetPwdForm" class="space-y-4" action="{{ route('reset_password') }}" method="POST">
    @csrf
    <div>
        <label for="res_email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('login_form.email') }} *</label>
    ...
    <a href="#" x-on:click="showloginForm = true" class="text-sm text-green-gs hover:underline hover:text-amber-300 dark:text-green-gs">{{ __('login_form.back_to_login') }}</a>
    <button type="submit" class="w-full text-white bg-green-gs hover:bg-amber-300 hover:text-green-gs focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-gs dark:hover:bg-green-gs/30">
        <svg x-show="loadingRequest" aria-hidden="true" class="inline w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-green-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
