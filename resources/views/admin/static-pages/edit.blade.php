    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <form action="{{ route('static.update', $staticPage) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="title" class="block text-sm font-medium text-gray-700 mb-1">{{ __('static_pages.title') }}</label>
                    <input type="text" name="title" id="title" value="{{ old('title', $staticPage->title) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>

                <div class="mb-6" wire:ignore>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('static_pages.content') }}</label>
                    <div wire:ignore>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="meta_title" class="block text-sm font-medium text-gray-700 mb-1">{{ __('static_pages.meta_title') }}</label>
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $staticPage->meta_title) }}"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label for="meta_description" class="block text-sm font-medium text-gray-700 mb-1">{{ __('static_pages.meta_description') }}</label>
