  ...
  <div x-data="articleManager()" x-init="init()" class="py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-6 bg-white p-4 rounded-lg shadow">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
              <!-- Status Filter -->
              <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{__('article_management.status')}}</label>
              ...
              <!-- Category Filter -->
              <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{__('article_management.category')}}</label>
              ...
              <div class="md:col-span-2">
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{__('article_management.date')}}</label>
      ...
      <div class="bg-white shadow overflow-hidden sm:rounded-lg">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                  <template x-for="article in filteredArticles" :key="article.id">
                      <tr>
                          ...
                          <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                              <span class="mr-2 text-sm font-medium text-gray-700" x-text="article.is_published ? '{{__('article_management.published')}}' : '{{__('article_management.drafts')}}'"></span>
          ...
          <div class="px-6 py-4 border-t border-gray-200" x-show="articles.length > 0">
              <div class="flex justify-between items-center">
                  <div x-text="`{{__('article_management.showing')}} ${startItem} {{__('article_management.to')}} ${endItem} {{__('article_management.of')}} ${totalItems} {{__('article_management.articles')}}`"></div>
                  <div class="flex space-x-2">
                      <button @click="prevPage" :disabled="currentPage === 1"
                              :class="currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''"
                              class="px-3 py-1 border rounded-md">{{__('article_management.previous')}}</button>
                      <button @click="nextPage" :disabled="currentPage === totalPages"
                              :class="currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''"
                              class="px-3 py-1 border rounded-md">{{__('article_management.next')}}</button>
                  </div>
              </div>
          </div>
      </div>
          ...
          <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div x-show="showDeleteModal" x-transition
                  ...
                  <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                      <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">{{__('article_management.confirm_deletion')}}</h3>
                      <p class="text-gray-600">{{__('article_management.confirm_deletion_message', ['title' => '<span x-text="selectedArticle?.title"></span>'])}}</p>
                  </div>
