...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="rounded-xl bg-white p-6 shadow-lg sm:p-8">
            <h2 class="mb-6 border-b-2 border-supaGirlRose pb-4 text-2xl font-bold text-green-gs font-roboto-slab">
            ...
            <form x-data="visibility()" method="POST" action="{{ route('profile.visibility.update') }}" class="space-y-6">
                ...
                <div class="space-y-4">
                    {{-- Option Public --}}
                    <div
                        class="relative flex items-start rounded-lg border-2 border-transparent bg-fieldBg p-4 transition-all duration-200 hover:border-supaGirlRose/50">
                        <div class="flex h-5 items-center">
                            <input x-on:click="customVisibility=false" type="radio" name="visibility" value="public"
                                ...
                                class="h-5 w-5 border-gray-300 text-green-gs focus:ring-green-gs/50">
                        </div>
                        <div class="ml-3 flex flex-col">
                            <label for="public-visibility"
                                class="block text-sm font-medium text-green-gs font-roboto-slab">{{ __('visibility.public.label') }}</label>
                            <span class="mt-1 block text-sm text-textColorParagraph font-roboto-slab">{{ __('visibility.public.description') }}</span>
                        </div>
                    </div>

                    {{-- Option Private --}}
                    <div
                        class="relative flex items-start rounded-lg border-2 border-transparent bg-fieldBg p-4 transition-all duration-200 hover:border-supaGirlRose/90">
                        <div class="flex h-5 items-center">
                        ...
                        <div class="ml-3 flex flex-col">
                            <label for="private-visibility"
                                class="block text-sm font-medium text-green-gs font-roboto-slab">{{ __('visibility.private.label') }}</label>
                            <span class="mt-1 block text-sm text-textColorParagraph font-roboto-slab">{{ __('visibility.private.description') }}</span>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="ml-3 flex flex-col">
                            <label for="custom-visibility"
                                class="block text-sm font-medium text-green-gs font-roboto-slab">{{ __('visibility.custom.label') }}</label>
                            <span class="mt-1 block text-sm text-textColorParagraph font-roboto-slab">{{ __('visibility.custom.description') }}</span>
                        </div>
                    </div>

                    {{-- Country Selector --}}
                    <div x-show="customVisibility" x-collapse
                        class="mt-6 space-y-4 rounded-lg border-l-4 border-green-gs/50 bg-fieldBg p-6 pl-8">
                        <h3 class="text-lg font-semibold text-green-gs font-roboto-slab">
                            {{ __('visibility.country_selector.title') }}
                        </h3>
                        <p class="text-sm text-textColorParagraph font-roboto-slab">
                        ...
                        <div x-data="countrySelector({{ Js::from(config('countries')) }}, {{ Js::from(auth()->user()->visible_countries ?? []) }})" class="relative">
                            <label
                                class="mb-1 block text-sm font-medium text-green-gs font-roboto-slab">{{ __('visibility.country_selector.title') }}</label>

                            <div
                                class="flex min-h-[48px] flex-wrap items-center rounded-xl border border-2 border-green-gs/50 bg-white px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-green-gs/50">
                                <template x-for="(code, index) in selected" :key="code">
                                    <div
                                        class="mb-2 mr-2 flex items-center rounded-full bg-fieldBg px-3 py-1 text-sm text-green-gs">
                                        <span x-text="countries[code]"></span>
                                        <button type="button" @click="remove(code)"
                                            class="ml-1 text-green-gs hover:text-green-gs">&times;</button>
                                ...
                                <input type="text" x-model="search" @focus="open = true"
                                    ...
                                    class="min-w-[120px] flex-1 border-none text-sm placeholder-gray-400 font-roboto-slab text-sm focus:ring-0"
