...
<div x-data="{'InviteForm': true}" class="relative w-full overflow-x-hidden">
    ...
    <div x-data="{'registerForm':''}"
        ...
        <div class="w-full xl:w-1/2 mx-auto px-2 pb-5 pt-30 xl:pt-0 xl:px-30 flex flex-col items-center justify-center gap-20">
            <h2 class="font-dm-serif text-2xl font-bold text-center">{{__('register_form.become_member')}}</h2>
            ...
            <form class="flex w-full mx-auto flex-col gap-5" action="{{ route('register') }}" method="POST">
                ...
                <div class="font-dm-serif font-bold">{{__('register_form.terms_conditions')}} <br> {{__('register_form.see_terms')}} <a class="text-green-gs" href="{{ route('static.cgv') }}">{{__('register_form.terms_conditions_link')}}</a></div>
                ...
                <div class="flex items-start mb-5">
                    ...
                    <label for="cgu_accepted" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300 @error('cgu_accepted') text-red-300 dark:text-red-500 @enderror">{{__('register_form.accept_terms')}}</label>
                </div>

                <button type="submit" class="text-white bg-amber-500 hover:bg-amber-600 focus:ring-4 focus:outline-none focus:ring-amber-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-amber-600 dark:hover:bg-amber-500 dark:focus:ring-amber-600">{{__('register_form.register')}}</button>
            </form>
        </div>
