...
<div class="relative w-full overflow-x-hidden">
    ...
    <div class="w-full h-full flex items-center justify-center">
        ...
        <div class="w-full xl:w-1/2 mx-auto py-3 px-2 xl:px-30 flex flex-col items-center justify-center gap-15">
            <h2 class="font-dm-serif text-2xl font-bold text-center">{{__('salon_register_form.register_salon')}}</h2>
            ...
            <form x-show="!escortForm" class="w-full mx-auto flex flex-col gap-3" action="{{ route('register') }}" method="POST">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <label for="floating_pass_conf" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-green-gs peer-focus:dark:text-green-gs peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 @error('password') text-red-700 dark:text-red-500 peer-focus:text-red-700 peer-focus:dark:text-red-500 @enderror">{{__('salon_register_form.confirm_password')}} *</label>
                    @error('password')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{__('salon_register_form.oops')}}</span> {{ $message }}</p>
                    @enderror
                </div>

                <div class="font-dm-serif font-bold">{{__('salon_register_form.terms_conditions')}} <br> {{__('salon_register_form.see_terms')}} <a class="text-green-gs" href="{{ route('static.cgv') }}">{{__('salon_register_form.terms_conditions_link')}}</a></div>
                ...
                <div class="flex items-start mb-5">
                    ...
                    <label for="cgu_accepted" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300 @error('cgu_accepted') text-red-300 dark:text-red-500 @enderror">{{__('salon_register_form.accept_terms')}}</label>
                </div>

                <button type="submit" class="text-white bg-amber-500 hover:bg-amber-600 focus:ring-4 focus:outline-none focus:ring-amber-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-amber-600 dark:hover:bg-amber-500 dark:focus:ring-amber-600">{{__('salon_register_form.register')}}</button>
            </form>
