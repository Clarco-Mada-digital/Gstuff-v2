    ...
    <div x-data="{}" class="container mx-auto flex flex-col justify-center xl:flex-row">
        ...
        <div class="min-w-1/4 -mt-26 flex flex-col items-center gap-3 px-4">
            ...
            <div class="ml-3 flex flex-col items-center justify-center">
                    ...
                    <p class="font-roboto-slab flex items-center gap-2 font-bold">
                        ...
                        @if ($escort->profile_verifie === 'verifier')
                            <span class="group relative flex items-center justify-center"
                                ...
                                <span
                                    class="absolute bottom-full mb-2 whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-300 group-hover:opacity-100">
                ...
                <p class="{{ $escort->isOnline() ? 'text-green-gs' : 'text-gray-500' }} font-roboto-slab text-xs">
            ...
            <section>
                    ...
                    <div class="flex items-center gap-5 py-5">
                        ...
                        <div class="flex items-center gap-5">
                            @foreach ($escort->getCategoriesAttribute() as $category)
                                <x-service-badge :text="$category->getTranslation('nom', app()->getLocale())" color="green-gs" hoverColor="fieldBg"
                ...
                <div class="flex flex-wrap items-center gap-10">
                    <div class="grid w-full grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <x-profile-info-item icon="origine_icon.png" :alt="__('escort_profile.origin_icon')" :label="__('escort_profile.origin')"
                            :value="$escort->origine" />



                        <x-profile-info-item icon="yeux_icon.png" :alt="__('escort_profile.eye_color_icon')" :label="__('escort_profile.eye_color')" :value="$escort->couleurYeux"
                            translation-path="name" />


                        <x-profile-info-item icon="cheveux_icon.png" :alt="__('escort_profile.hair_color_icon')" :label="__('escort_profile.hair_color')"
                            :value="$escort->couleurCheveux" translation-path="name" />

                        <x-profile-info-item icon="tarif_icon.png" :alt="__('escort_profile.rate_icon')" :label="__('escort_profile.rates_from')" :value="$escort->tarif"
                            suffix="CHF" />

                        <x-profile-info-item icon="taille_icon.png" :alt="__('escort_profile.height_icon')" :label="__('escort_profile.height')" :value="$escort->tailles"
                            suffix="cm" />

                        <x-profile-info-item icon="poitrine_icon.png" :alt="__('escort_profile.bust_icon')" :label="__('escort_profile.bust')"
                            :value="$escort->poitrine" translation-path="name" />

                        <x-profile-info-item icon="mobilite.png" :alt="__('escort_profile.mobility_icon')" :label="__('escort_profile.mobility')" :value="$escort->mobilite"
                            translation-path="name" />

                        <x-profile-info-item icon="mensuration.png" :alt="__('escort_profile.measurements_icon')" :label="__('escort_profile.measurements')" :value="$escort->mensuration"
                            translation-path="name" />

                        <x-profile-info-item icon="taill_poit.png" :alt="__('escort_profile.bust_size_icon')" :label="__('escort_profile.bust_size')" :value="$escort->poitrine"
                            translation-path="name" :suffix="$escort->poitrine ? __('escort_profile.cup') : ''" />
                ...
                {{-- Description --}}
                @if ($escort->apropos)
                    <x-profile.description :title="__('escort_profile.description')" :content="$escort->apropos" />
                @endif
                {{-- Services --}}
                @if ($escort->services->isNotEmpty())
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        @foreach ($escort->services as $service)
                            <x-service-badge :text="$service->getTranslation('nom', app()->getLocale())" color="green-gs" hoverColor="fieldBg"
                                borderColor="supaGirlRose" bgColor="fieldBg" textHoverColor="fieldBg" />
                        @endforeach
