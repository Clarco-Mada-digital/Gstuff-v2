...
@section('content')
    <div x-data="{}"
        x-on:click="$dispatch('img-modal', {  imgModalSrc: '{{ $couverture_image = $escort->couverture_image }}' ? '{{ asset('storage/couvertures/' . $couverture_image) }}' : '{{ asset('images/Logo_lg.svg') }}', imgModalDesc: '' })"
        class="relative w-full max-h-[30vh] min-h-[30vh] overflow-hidden"
        style="background: url({{ $escort->couverture_image ? asset('storage/couvertures/' . $escort->couverture_image) : asset('images/Logo_lg.svg') }}) center center /cover;">
    </div>
    ...
    <div x-data="{}" class="container flex flex-col xl:flex-row justify-center mx-auto">
        ...
        <div class="min-w-1/4 flex flex-col items-center gap-3">
            ...
            <div class="w-55 h-55 relative -translate-y-[50%] rounded-full border-5 border-white mx-auto">
                ...
                <span
                    class="absolute bottom-4 right-5 block w-3 h-3 rounded-full
                    {{ $escort->isOnline() ? 'bg-green-gs' : 'bg-gray-400' }} ring-2 ring-white">
                </span>
            </div>
            <div class="ml-3 flex flex-col justify-center items-center -mt-[25%] md:-mt-[10%] xl:-mt-[25%]">
                <p class="font-bold flex items-center gap-2">{{ Str::ucfirst($escort->prenom) }} @if ($escort->profile_verifie == 'verifier')
                    <svg xmlns="http://www.w3.org/2000/svg" title="{{__('escort_profile.verified_profile')}}" class="w-5 h55 inline-block text-green-gs" viewBox="0 0 24 24"><path fill="currentColor" d="M12 4c-4.41 0-8 3.59-8 8s3.59 8 8 8s8-3.59 8-8s-3.59-8-8-8m-2 13l-4-4l1.41-1.41L10 14.17l6.59-6.59L18 9z" opacity=".3"/><path fill="currentColor" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2m0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8s8 3.59 8 8s-3.59 8-8 8m4.59-12.42L10 14.17l-2.59-2.58L6 13l4 4l8-8z"/></svg>
                @endif</p>
                <p class="text-sm {{ $escort->isOnline() ? 'text-green-gs' : 'text-gray-500' }}">
                    ({{ $escort->last_seen_for_humans }})
                </p>
            </div>
            <span class="flex items-center gap-2 font-bold font-dm-serif"><svg class="w-5 h-5"
            ...
            <a href="tel:{{ $escort->telephone ?? '' }}" class="flex items-center gap-2 font-bold font-dm-serif">
            ...
            @endauth
            <button @auth x-on:click="$dispatch('loadForSender', [{{ $escort->id }}])" @else data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" @endauth
                class="flex items-center justify-center gap-2 w-full p-2 text-green-gs text-sm rounded-lg border border-gray-400 cursor-pointer hover:bg-green-gs hover:text-white">
                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path fill="currentColor"
        ...
        @endphp
            <a href="mailto:{{ $escort->email }}"
                ...
                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path fill="currentColor"
                        d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10h5v-2h-5c-4.34 0-8-3.66-8-8s3.66-8 8-8s8 3.66 8 8v1.43c0 .79-.71 1.57-1.5 1.57s-1.5-.78-1.5-1.57V12c0-5.52-4.48-10-10-10m0 13c-1.66 0-3-1.34-3-3s1.34-3 3-3s3 1.34 3 3s-1.34 3-3 3" />
                </svg>
        ...
        <div class="min-w-3/4 px-5 py-5">
            ...
            <section>
                ...
                <div class="flex items-center gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.category')}}</h2>
                ...
                <div class="flex items-center justify-between gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.stories')}}</h2>
                ...
                <div class="flex items-center justify-between gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.about_me')}}</h2>
                    <div class="flex-1 h-0.5 bg-green-gs"></div>
                ...
                <div class="flex items-center gap-10 flex-wrap">
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 w-full">
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/age_icon.svg') }}" alt="{{__('escort_profile.age_icon')}}" />
                            <span>{{__('escort_profile.age')}} : {{ Carbon::parse($escort->date_naissance)->age }} {{__('escort_profile.years_old')}}</span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/origine_icon.svg') }}" alt="{{__('escort_profile.origin_icon')}}" />
                            <span>{{__('escort_profile.origin')}} : {{ $escort->origine ?? '-' }} </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/langue_icon.svg') }}" alt="{{__('escort_profile.language_icon')}}" />
                            <span>
                        ...
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/yeux_icon.svg') }}" alt="{{__('escort_profile.eye_color_icon')}}" />
                            <span>{{__('escort_profile.eye_color')}} : {{ $escort->couleur_yeux ?? '-' }} </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/cheveux_icon.svg') }}" alt="{{__('escort_profile.hair_color_icon')}}" />
                            <span>{{__('escort_profile.hair_color')}} : {{ $escort->couleur_cheveux ?? '-' }} </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/tarif_icon.svg') }}" alt="{{__('escort_profile.rate_icon')}}" />
                            @if ($escort->tarif)
                        ...
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/taille_icon.svg') }}" alt="{{__('escort_profile.height_icon')}}" />
                            <span>{{__('escort_profile.height')}} : {{ $escort->tailles ?? '-' }} cm </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/poitrine_icon.svg') }}" alt="{{__('escort_profile.bust_icon')}}" />
                            <span>{{__('escort_profile.bust')}} : {{ $escort->poitrine ?? '-' }} </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/mobilite.svg') }}" alt="{{__('escort_profile.mobility_icon')}}" />
                            <span>{{__('escort_profile.mobility')}} : {{ $escort->mobilite ?? '-' }}</span>
                        </div>

                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/mensuration.svg') }}" alt="{{__('escort_profile.measurements_icon')}}" />
                            <span>{{__('escort_profile.measurements')}} : {{ $escort->mensuration ?? '-' }}</span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/taill_poit.svg') }}" alt="{{__('escort_profile.bust_size_icon')}}" />
                            <span>{{__('escort_profile.bust_size')}} : {{__('escort_profile.cup')}} {{ $escort->poitrine ?? '-' }} </span>
                        </div>
                        <div class="w-full flex items-center gap-3 font-dm-serif">
                            <img src="{{ asset('images/icons/cart_icon.svg') }}" alt="{{__('escort_profile.payment_icon')}}" />
                            <span>
                ...
                <div class="flex items-center justify-between gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.description')}}</h2>
                    <div class="flex-1 h-0.5 bg-green-gs"></div>
                ...
                <div class="flex items-center justify-between gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.services_offered')}}</h2>
                    <div class="flex-1 h-0.5 bg-green-gs"></div>
                ...
                <div class="flex items-center justify-between gap-5 py-5">
                    ...
                    <h2 class="font-dm-serif font-bold text-2xl text-green-gs">{{__('escort_profile.associated_salon')}}</h2>
                    <div class="flex-1 h-0.5 bg-green-gs"></div>
                ...
                <div class="w-full flex items-center gap-10 flex-wrap">
                    ...
                    @if ($salonAssociers->isNotEmpty())
                        ...
                        @endforeach
                    @else
                        <span class="w-full text-center text-green-gs font-bold font-dm-serif">{{__('escort_profile.no_associated_salon')}}</span>
                    @endif
                ...
                @guest
                    <div class="w-full flex flex-col items-center justify-center font-dm-serif gap-5 text-green-gs my-3">
                        ...
                        <p class="font-extrabold text-3xl text-center">{{__('escort_profile.connect_to_view_private_content')}}</p>
                        <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal"
                            class="font-dm-serif font-bold btn-gs-gradient rounded-lg">{{__('escort_profile.connect_signup')}}</button>
                    </div>
                @endguest
                @auth
